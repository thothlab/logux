...
**Android Logs & Traffic CLI** -- TUI-утилита для Android-разработчиков: просмотр логов в реальном времени с двухстрочной раскладкой, фильтрация, инспекция трафика и подмена сетевых ответов.
...
- **Два режима раскладки** -- `/layout linear` (по умолчанию): шапка timestamp/level/tag + message на следующей строке с переносами; `/layout compact`: все поля в одной строке в фиксированных колонках, message обрезается по оставшейся ширине
...
### Вариант 2: скачать готовый бинарник вручную
...
| `/layout linear\|compact` | Переключить раскладку: **linear** — шапка + message ниже; **compact** — всё на одной строке в колонках |
...
src/
 ...
 ├── cli/
 │   ├── tui.rs            -- TUI: ratatui, event loop, двухстрочный рендеринг (шапка + message)
...
| Тег | Язык | Описание |
|-----|------|----------|
| `v2.1.0` | Rust | TUI с двухстрочной раскладкой (шапка + message), устойчивый стрим (UTF-8 lossy), авто-переподключение, `/reconnect` |
