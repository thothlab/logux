...
| Клавиша | Действие |
|---------|----------|
| `Колёсико мыши` | Прокрутка логов по 1 строке (нужен `/mouse on`) |
...
**Выделение мышью и копирование:** работает по умолчанию (capture мыши выключен). Чтобы включить прокрутку колёсиком — `/mouse on`. В этом режиме для выделения текста нужно удерживать Option/Alt при перетаскивании (на macOS).
